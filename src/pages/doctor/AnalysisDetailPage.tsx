...
/**
 * Read-only analysis detail for doctors. Mirrors the patient's detail page
 * but pulls from the doctor-scoped endpoint and surfaces the patient context
 * + a quick-action to switch into validation mode when applicable.
 */
export default function DoctorAnalysisDetailPage() {
  ...
  const canValidate = data.status === 'COMPLETED' || data.status === 'REQUIRES_REVIEW';

  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center gap-3 flex-wrap">
        <Link to="/doctor/assigned-analyses">
        ...
        {canValidate && (
          <Button
            ...
            className="ml-auto"
            onClick={() => navigate(`/doctor/analyses/${data.id}/validate`)}
          >
      ...
      <Card>
        ...
        {data.patientName && (
          <div className="mt-4 flex items-center gap-2 text-sm">
            ...
            <span className="text-gray-500">{t('validate.patient_label')}:</span>
            {data.patientId ? (
              <Link
                to={`/doctor/patients/${data.patientId}`}
