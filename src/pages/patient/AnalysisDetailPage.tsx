...
export default function AnalysisDetailPage() {
  ...
  const { data, isLoading, refetch, isFetching } = useQuery({
    ...
    queryFn: async () => {
      const r = await xraysApi.getOne(id!);
      return r.data.data!;
    },
  ...
  if (!data)
    return (
      <div className="text-center py-16 text-gray-400">
        {t("analyses.not_found")}
      </div>
    );

  const confidence =
    data.aiConfidence != null ? Math.round(data.aiConfidence * 100) : null;
      ...
      <Card>
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-3 mt-1 text-sm text-gray-500 flex-wrap">
              ...
              {data.fileSizeBytes && (
                <span>{(data.fileSizeBytes / 1024 / 1024).toFixed(2)} MB</span>
              )}
              <span>
                {t("analyses.uploaded_at")}:{" "}
                {format(new Date(data.uploadedAt), "dd/MM/yyyy, HH:mm", {
                  locale: dateLocale,
                })}
              </span>
            </div>
        ...
        {data.patientNotes && (
          <div className="mt-4 p-3 bg-gray-50 rounded-lg text-sm text-gray-600">
            <span className="font-medium">{t("analyses.notes_label")}: </span>
            {data.patientNotes}
          </div>
        )}
      </Card>
        ...
        <Card>
          <div className="py-8 text-center">
            ...
            <p className="font-semibold text-gray-900">
              {t("analyses.processing_title")}
            </p>
            <p className="text-gray-500 text-sm mt-1">
              {t("analyses.processing_sub")}
            </p>
            <Button
              variant="outline"
              size="sm"
      ...
      {data.aiPrimaryDiagnosis && (
        <Card>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
            <div className="p-4 bg-blue-50 rounded-xl">
              <p className="text-xs text-blue-600 font-medium uppercase tracking-wide mb-1">
                {t("analyses.diagnosis_label")}
              </p>
              <p className="font-semibold text-gray-900">
                {t("disease." + data.aiPrimaryDiagnosis)}
              </p>
            </div>
            {confidence != null && (
              <div className="p-4 bg-green-50 rounded-xl">
                <p className="text-xs text-green-600 font-medium uppercase tracking-wide mb-1">
                  {t("analyses.confidence_label")}
                </p>
          ...
          {data.aiDetectedAbnormalities && (
            <div>
              <p className="text-sm font-medium text-gray-700 mb-1">
                {t("analyses.abnormalities_label")}
              </p>
              <p className="text-sm text-gray-600 bg-gray-50 rounded-lg p-3 whitespace-pre-wrap">
                {data.aiDetectedAbnormalities}
              </p>
          ...
          {data.analyzedAt && (
            <p className="text-xs text-gray-400 mt-4">
              {t("analyses.analyzed_at")}:{" "}
              {format(new Date(data.analyzedAt), "dd/MM/yyyy, HH:mm", {
                locale: dateLocale,
              })}
            </p>
          )}
            ...
            <div className="p-4 bg-gray-50 rounded-xl">
              ...
              {data.validatedAt && (
                <p className="text-xs text-gray-500 mt-0.5">
                  {format(new Date(data.validatedAt), "dd/MM/yyyy", {
                    locale: dateLocale,
                  })}
                </p>
              )}
